...
int main() {
  ...
  int dsamp = 100;  //sampling rate[Hz]
  ...
  // wave_acc = input_wave::wavedata(tim,"input/data/wavdata.in");
  ...
  // --- Write output file --- //
    if (it%(fsamp/dsamp) == 0){
      ...
      if (it%(fsamp/dsamp*10) == 0){
        Element* out_element_p = &fem.elements[1];
        std::cout << " t=" << it*dt << " " << out_element_p->eff_stress(1) << " " << out_element_p->excess_pore_pressure << std::endl;
      }
    }
  }
