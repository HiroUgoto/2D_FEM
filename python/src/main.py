...
output_element_excess_pore_pressure = np.zeros((ntim,fem.output_nelem))
...
for it in range(ntim):
    ...
    output_element_excess_pore_pressure[it,:] = [element.excess_pore_pressure for element in fem.output_elements]
...
output_line = tim[:ntim]
for ielem in range(fem.output_nelem):
    output_line = np.vstack([output_line,output_element_excess_pore_pressure[:,ielem]])
np.savetxt(output_dir+"output_element.excess_pore_pressure",output_line.T)
